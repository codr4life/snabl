...
namespace snabl {
	...
	public:
		...
		std::optional<std::size_t> score(const Stack &stack) const override;
	};
	...
	template <int NARGS, int NRETS>
	std::optional<std::size_t> Fimp<NARGS, NRETS>::score(const Stack &stack) const {
		if (!NARGS) { return 0; }
		if (stack.size() < NARGS) { return std::nullopt; }
